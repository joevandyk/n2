...
class PostgresDbWriter < PostgresWriter
  ...
  def open
    ...
    @conn.exec("SET search_path TO #{schema}") if schema
  ...
  def write_table(table)
    ...
    if @conn.server_version < 80200
      @conn.exec "DROP TABLE #{table.name} CASCADE;" if exists?(table.name)
    else
      @conn.exec "DROP TABLE IF EXISTS #{table.name} CASCADE;"
    end
  ...
  def write_indexes(table)
    ...
    if primary_index = table.indexes.find {|index| index[:primary]}
      @conn.exec("ALTER TABLE #{table.name} ADD CONSTRAINT \"#{table.name}_pkey\" PRIMARY KEY(#{primary_index[:columns].map {|col| col}.join(", ")})")
    end
    ...
    table.indexes.each do |index|
      ...
      if @conn.server_version < 80200
        @conn.exec("DROP INDEX #{indexname} CASCADE;") if exists?(indexname)
      else
        @conn.exec("DROP INDEX IF EXISTS #{indexname} CASCADE;")
      end
      @conn.exec("CREATE #{unique} INDEX #{indexname[0..63]} ON #{table.name} (#{index[:columns].map {|col| col}.join(", ")});")
    end


    #@conn.exec("VACUUM FULL ANALYZE #{table.name}")
  ...
  def write_constraints(table)
    table.foreign_keys.each do |key|
      key_sql = "ALTER TABLE #{table.name} ADD FOREIGN KEY (#{key[:column].map{|c| c}.join(', ')}) REFERENCES #{key[:ref_table]}(#{key[:ref_column].map{|c|c}.join(', ')}) ON UPDATE #{key[:on_update]} ON DELETE #{key[:on_delete]}"
      begin
        @conn.exec(key_sql)
      rescue Exception => e
        puts "Error: \n#{key_sql}\n#{e}"
  ...
  def write_contents(table, reader)
    _time1 = Time.now
    copy_line = "COPY #{table.name} (#{table.columns.map {|column| column[:name]}.join(", ")}) FROM stdin;"
    @conn.exec(copy_line)
